...
from audio.common import DCOffset, wait_for_port, exp_mov_avg, BAUD_RATE, MicReader, RateEstimator
...
def main(port: str | None = None, baud: int = BAUD_RATE) -> None:
    ...
    axes[0].set_title(f"Spike profile — {PRE} pre / {POST} post samples @ {rate} Hz")
    axes[0].set_ylabel("Amplitude")
    axes[0].legend()

    axes[1].specgram(samples, Fs=rate, cmap='inferno')
